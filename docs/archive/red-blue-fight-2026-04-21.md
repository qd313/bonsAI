...
| Near-term      | Prompt testing and tuning                                                   | ★★     | **defer** — process-heavy; belongs post-release with [testing.md](testing.md) matrix ownership.                                                                                      | **Defer** — improves quality narrative but not blocking honest ship if prompts unchanged.                                                              |
...
| Planned        | System prompt reorder + general-purpose assistant clause                    | ★★★    | **defer** — `build_system_prompt` refactor affects every Ask; needs staged rollout + [testing.md](testing.md).                                                                       | **Defer** — trust lives in predictable JSON/TDP tail; do not reorder without documented contract tests.                                                |
...
- **Week work list (after the bell):**
  ...
  4. **Prompt testing and tuning** — run/extend matrices in [testing.md](testing.md); log results for ship sign-off.
