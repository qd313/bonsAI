...
2. **Screenshots** — Run `scripts/screenshot-deck.ps1` or `scripts/screenshot-deck.sh` (auto-detects game vs desktop on the Deck). Saves `screenshots/DeckCapture_<timestamp>_<mode>.png`. Agents read newest files per `.cursor/skills/decky-screenshot-ingest/SKILL.md`.
