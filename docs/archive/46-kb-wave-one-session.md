...
|---|---|---|---|---|
| **R1 Corpus update through the plugin** | Old corpus installed | Ollama tab → *Update knowledge base* | The tab shows the new version (the release date); a game question's Show details names it; 266 cards reachable (probe if needed) | `runs/W1-R1-update.json` **[no evidence — re-run, batch QA-EVIDENCE-GAP-01]**, plugin log tail |
